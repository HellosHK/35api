...
""")
print("             [+]​YT.T|A|I|W|A|N😎")
print("             [+]​Facebook:ชัช 'ช // ไต้หวัน ก็อตต🎭")
print("             [+]​35api😈​")
phone = input("Phone :> ")
...
for i in range(num):
    ...
    threading.Thread(target=API_30).start()
    threading.Thread(target=API_31).start()
    threading.Thread(target=API_32).start()
    threading.Thread(target=API_33).start()
    threading.Thread(target=API_34).start()
    threading.Thread(target=API_35).start()